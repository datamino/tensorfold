
import { Component, ChangeDetectionStrategy, signal } from '@angular/core';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'app-footer',
  standalone: true,
  imports: [CommonModule],
  template: `
    <footer class="relative bg-slate-900 pt-24 pb-0 overflow-hidden border-t border-slate-800">
      
      <!-- Content Grid -->
      <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-12 mb-12 relative z-10">
        
        <!-- Brand Column (4 cols) -->
        <div class="md:col-span-4 flex flex-col items-start">
          <div class="mb-8 flex items-center gap-2">
             <span class="w-3 h-3 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,211,238,0.8)]"></span>
             <span class="font-bold tracking-tight text-white text-xl">TensorFold</span>
          </div>
          <p class="text-slate-300 font-light leading-relaxed mb-8 max-w-sm">
            We engineer the neural architectures that power the next generation of intelligent enterprise.
            <br/><br/>
            Fold intelligence into reality.
          </p>
          
          <!-- Socials: White Buttons with Dark Icons -->
          <div class="flex gap-4">
             <a href="#" class="group w-10 h-10 rounded-full bg-white flex items-center justify-center text-slate-900 hover:bg-cyan-400 hover:text-slate-900 transition-all duration-300 shadow-lg hover:shadow-cyan-400/50 hover:-translate-y-1">
               <span class="sr-only">Twitter</span>
               <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"></path></svg>
             </a>
             <a href="#" class="group w-10 h-10 rounded-full bg-white flex items-center justify-center text-slate-900 hover:bg-cyan-400 hover:text-slate-900 transition-all duration-300 shadow-lg hover:shadow-cyan-400/50 hover:-translate-y-1">
               <span class="sr-only">GitHub</span>
               <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" clip-rule="evenodd"></path></svg>
             </a>
             <a href="#" class="group w-10 h-10 rounded-full bg-white flex items-center justify-center text-slate-900 hover:bg-cyan-400 hover:text-slate-900 transition-all duration-300 shadow-lg hover:shadow-cyan-400/50 hover:-translate-y-1">
               <span class="sr-only">LinkedIn</span>
               <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clip-rule="evenodd"></path></svg>
             </a>
          </div>
        </div>

        <!-- Links Column 1 -->
        <div class="md:col-span-2">
          <h4 class="font-mono text-xs uppercase tracking-widest text-cyan-400 mb-6 font-bold">Platform</h4>
          <ul class="space-y-4">
            @for (link of platformLinks(); track link) {
              <li><a href="#" class="text-slate-300 hover:text-white transition-colors text-sm font-light tracking-wide">{{link}}</a></li>
            }
          </ul>
        </div>

        <!-- Links Column 2 -->
        <div class="md:col-span-2">
          <h4 class="font-mono text-xs uppercase tracking-widest text-cyan-400 mb-6 font-bold">Company</h4>
          <ul class="space-y-4">
            @for (link of companyLinks(); track link) {
              <li><a href="#" class="text-slate-300 hover:text-white transition-colors text-sm font-light tracking-wide">{{link}}</a></li>
            }
          </ul>
        </div>

        <!-- Newsletter -->
        <div class="md:col-span-4">
           <h4 class="font-mono text-xs uppercase tracking-widest text-cyan-400 mb-6 font-bold">Stay Synced</h4>
           <p class="text-slate-300 text-sm mb-6 font-light">Receive quarterly transmissions from our research labs.</p>
           
           <!-- White Input Style -->
           <form class="flex gap-0 relative group shadow-2xl">
             <input type="email" placeholder="Email Address" class="w-full bg-white border-0 rounded-l-lg pl-6 pr-4 py-4 text-sm focus:outline-none text-slate-900 placeholder:text-slate-400 shadow-inner">
             <button class="bg-cyan-500 hover:bg-cyan-400 text-slate-900 px-8 rounded-r-lg text-xs font-bold uppercase tracking-wider transition-colors">
               Join
             </button>
           </form>
           
           <div class="mt-6 flex items-center gap-2 text-[10px] text-slate-400">
             <span class="w-1.5 h-1.5 rounded-full bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.8)]"></span>
             <span class="text-slate-300">All systems operational</span>
           </div>
        </div>
      </div>

      <!-- MASSIVE TEXT ABOVE BOTTOM LINE -->
      <!-- Adjusted: text-[13vw], translate-y-2/4, and added -ml-[1px] for slight left nudge -->
      <div class="w-full relative z-0 flex justify-center items-end border-b border-slate-800/50 pb-0 overflow-hidden">
        <h1 class="text-[20vw] font-bold text-white leading-none tracking-tighter select-none pointer-events-none drop-shadow-2xl w-full text-center translate-y-2 md:translate-y-4 -ml-[5px]">
          TensorFold
        </h1>
      </div>

      <!-- Bottom Bar -->
      <div class="relative bg-slate-950/30 z-20">
        <div class="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div class="text-xs text-slate-500 font-mono">
            © 2024 TensorFold Intelligence Inc. All rights reserved.
          </div>
          <div class="flex gap-8 text-xs text-slate-500">
            <a href="#" class="hover:text-white transition-colors">Privacy Policy</a>
            <a href="#" class="hover:text-white transition-colors">Terms of Service</a>
            <a href="#" class="hover:text-white transition-colors">Sitemap</a>
          </div>
        </div>
      </div>

    </footer>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FooterComponent {
  platformLinks = signal(['Custom AI Systems', 'Neural Architectures', 'Data Intelligence', 'Edge Inference', 'Security']);
  companyLinks = signal(['About', 'Research Labs', 'Careers', 'Press', 'Contact']);
}
